...
unary_sensitivities = (
    ...
    (cot,   :(-1 * csc(x)^2),                (lb, ub),          false),
    (sec,   :(1 * y * tan(x)),               (lb, ub),          true),
    (csc,   :(-1 * y * cot(x)),              (lb, ub),          true),
...
for (f, x̄, _, needs_y) in unary_sensitivities
    ...
    eval(AutoGrad2, :(∇(::typeof($f), ::Type{Arg{1}}, p, y, ȳ, x::Real) = ȳ * $x̄))
    eval(AutoGrad2, needs_y ?
        :(∇(::typeof($f), ::Type{Arg{1}}, x::Real, y) = $x̄) :
        :(∇(::typeof($f), ::Type{Arg{1}}, x::Real) = $x̄))
    eval(AutoGrad2, :(needs_output(::typeof($f)) = $needs_y))
end
...
for (f, x̄, ȳ, r1, r2) in binary_sensitivities
    eval(AutoGrad2, :(add_intercept($f, :Base)))
    eval(AutoGrad2, :(∇(::typeof($f), ::Type{Arg{1}}, p, z, z̄, x::Real, y::Real) = $x̄))
    eval(AutoGrad2, :(∇(::typeof($f), ::Type{Arg{2}}, p, z, z̄, x::Real, y::Real) = $ȳ))
end
