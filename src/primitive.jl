...
function primitive(f::Symbol, typepars::Tuple, argtypes::Tuple, diffs::Tuple)
    function primitive_(states::Vector{Bool}, pos::Int)
        if pos > length(states)
            if any(states .== true)
                eval(constructboxedfunc(f, typepars, argtypes, states))
            end
        else
            states[pos] = false
...
function constructboxedfunc(f::Symbol, tpars::Tuple, argts::Tuple, diffs::Vector{Bool})
    Expr(:(=), callexpr(f, tpars, argts, diffs), branchexpr(f, tpars, argts, diffs))
end
...
makebranch(f::Expr, t, a, d) = constructbranch(f.args[1], t, a, d)
function branchexpr(f::Symbol, typepars::Tuple, argtypes::Tuple, diffs::Vector{Bool})
    args = [Symbol("x", n) for n in eachindex(diffs)]
    return Expr(:call, :Branch, f, Expr(:tuple, args...), gettape(args, diffs))
end
function gettape(args::Vector, diffs::Vector{Bool})
