...
@testset "Diagonal" begin
    ...
    let rng = MersenneTwister(123456), N = 10
        ...
        for _ in 1:diagonal_P
            ...
            @test_broken check_errs(x->diagm(0 => x), diagm(0 => randn(rng, N)), x, vx)
            @test_broken check_errs(λ_2, λ_2(randn(rng, N)), x, vx)
            @test_broken check_errs(λ_m3, λ_m3(randn(rng, N)), x, vx)
            @test_broken check_errs(λ_0, λ_0(randn(rng, N)), x, vx)
            @test_broken check_errs(λ_false, λ_false(randn(rng, N)), x, vx)
            @test_broken check_errs(λ_true, λ_true(randn(rng, N)), x, vx)
        end
    end
end
