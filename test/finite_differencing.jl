...
import Nabla: init_rvs_tape, randned_container, zerod_container
...
function compute_Dv_update(
    ...
    # Randomly initialise the return sensitivities.
    ȳ = init_rvs_tape(y, ȳ)
    inits = Vector{Any}(undef, length(x))
    for n in eachindex(x)
        inits[n] = randned_container(y[n].value)
        ȳ[n] = copy(inits[n])
    end
